-- COMPREHENSIVE SECURITY HARDENING PACK
...
-- 5) Remove matviews from API access
DO $$
DECLARE r record;
BEGIN
  FOR r IN
  ...
  LOOP
    EXECUTE format('REVOKE ALL ON MATERIALIZED VIEW %I.%I FROM anon, authenticated;', r.schemaname, r.matviewname);
  END LOOP;
END$$;
...
-- 7) Pin search_path for SECURITY DEFINER functions that don't have it
DO $$
DECLARE
  r record;
BEGIN
  ...
  LOOP
    EXECUTE format(
      'ALTER FUNCTION %I.%I SET search_path TO ''public'', ''pg_temp'';',
      r.nspname, r.proname
    );
  END LOOP;
END$$;
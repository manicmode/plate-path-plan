...
SELECT 
  ...
  AVG(wl.skipped_sets) as avg_skipped_sets,
  AVG(wl.sets_completed::float / NULLIF(wl.target_sets, 0)) * 100 as completion_rate,
  COUNT(CASE WHEN wl.skipped_sets > 2 THEN 1 END)::float / COUNT(*) * 100 as high_skip_rate,
...
SELECT 
  ...
  AVG(wl.sets_completed::float / NULLIF(wl.target_sets, 0)) * 100 as avg_completion_rate,
  DATE_TRUNC('week', wl.completed_at) as week_start,
  CASE 
    WHEN AVG(wl.sets_completed::float / NULLIF(wl.target_sets, 0)) >= 0.9 THEN 'excellent'
    WHEN AVG(wl.sets_completed::float / NULLIF(wl.target_sets, 0)) >= 0.75 THEN 'good'
    WHEN AVG(wl.sets_completed::float / NULLIF(wl.target_sets, 0)) >= 0.6 THEN 'fair'
...
SELECT 
  ...
  ws.unique_exercises,
  ROUND((ws.total_sets::numeric / NULLIF(ws.total_sets + ws.total_skipped, 0)) * 100, 1) as completion_rate,
  ROUND((ws.total_skipped::numeric / NULLIF(ws.total_sets + ws.total_skipped, 0)) * 100, 1) as skip_rate,

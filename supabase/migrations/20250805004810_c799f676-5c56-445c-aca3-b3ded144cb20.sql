-- Fix security event logging for system events
-- Phase 3: Resolve system event logging constraint

-- 1. Update security event validation to allow system events
...
AS $$
BEGIN
  -- Allow system events (starting with 'system_' or 'SECURITY_HARDENING_') without user_id
  IF NEW.user_id IS NULL AND 
     NEW.event_type NOT LIKE 'system_%' AND 
     NEW.event_type NOT LIKE 'SECURITY_HARDENING_%' AND
     NEW.event_type NOT LIKE '%_COMPLETE' THEN
...
-- 2. Secure all remaining functions with proper search_path
...
-- 3. Create comprehensive client-side input validation enhancement
...
BEGIN
  ...
  ELSE
    ...
    -- Log security violation (only if we have a valid context)
...
-- 4. Create notification constraint validation function (enhanced)
...
-- 5. Log successful security hardening

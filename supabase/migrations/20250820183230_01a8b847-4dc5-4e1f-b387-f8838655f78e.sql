-- PHASE 3 (corrected): precise security touch-ups
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='habit_reminders') THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies
      WHERE schemaname='public' AND tablename='habit_reminders' AND policyname='habit_reminders_insert_own'
    ) THEN
      CREATE POLICY habit_reminders_insert_own
        ON public.habit_reminders FOR INSERT TO authenticated
        WITH CHECK (user_id = auth.uid());
    END IF;

    IF NOT EXISTS (
      SELECT 1 FROM pg_policies
      WHERE schemaname='public' AND tablename='habit_reminders' AND policyname='habit_reminders_update_own'
    ) THEN
      CREATE POLICY habit_reminders_update_own
        ON public.habit_reminders FOR UPDATE TO authenticated
        USING (user_id = auth.uid())
        WITH CHECK (user_id = auth.uid());
    END IF;
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies
      WHERE schemaname='public' AND tablename='habit_reminders' AND policyname='habit_reminders_delete_own'
    ) THEN
      CREATE POLICY habit_reminders_delete_own
        ON public.habit_reminders FOR DELETE TO authenticated
        USING (user_id = auth.uid());
    END IF;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='nudge_event' AND policyname='nudge_event_owner_all') THEN
    -- Policy already exists, skip
    NULL;
  ELSIF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='nudge_event') THEN

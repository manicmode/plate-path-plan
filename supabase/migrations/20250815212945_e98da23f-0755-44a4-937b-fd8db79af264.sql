...
BEGIN
  ...
  IF NOT v_exists THEN
    -- Get any existing user as creator or use a fixed UUID
    SELECT id INTO v_system_user_id FROM auth.users LIMIT 1;
    IF v_system_user_id IS NULL THEN
      v_system_user_id := '00000000-0000-0000-0000-000000000000'::uuid;
    END IF;
    
    INSERT INTO private_challenges (id, title, description, creator_id, category, status, start_date, duration_days, created_at)
    VALUES (gen_random_uuid(), 'Arena — Rank of 20', 'Auto-seeded active challenge', v_system_user_id,

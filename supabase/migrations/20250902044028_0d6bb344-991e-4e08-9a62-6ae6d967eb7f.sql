...
-- Add FK to auth.users only if not already present
do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'meal_sets_user_id_fkey'
  ) then
    alter table public.meal_sets
      add constraint meal_sets_user_id_fkey
      foreign key (user_id) references auth.users(id) on delete cascade;
  end if;
end$$;

-- Ensure items is a JSON array (replace if existed)
alter table public.meal_sets drop constraint if exists meal_sets_items_is_array;
alter table public.meal_sets
  add constraint meal_sets_items_is_array check (jsonb_typeof(items) = 'array');
...
create policy "Users can view their own meal sets"
  on public.meal_sets for select
...
create policy "Users can create their own meal sets"
  on public.meal_sets for insert
...
create policy "Users can update their own meal sets"
  on public.meal_sets for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
...
create policy "Users can delete their own meal sets"
  on public.meal_sets for delete
  using (auth.uid() = user_id);
...
-- Ensure user_id is set from auth.uid() (idempotent)

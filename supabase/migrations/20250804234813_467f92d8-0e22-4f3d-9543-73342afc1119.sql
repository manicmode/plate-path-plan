-- =============================================================================
-- CRITICAL SECURITY FIXES - Phase 1: Database Issues (UUID Cast Fixed)
-- =============================================================================
...
-- Fix 5: Add UUID validation constraints to critical tables with proper casting
DO $$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'challenge_invitations') THEN
    IF NOT EXISTS (
      ...
      CHECK (
        inviter_id::text ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' AND
        invitee_id::text ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
      );
    END IF;
  END IF;
END $$;

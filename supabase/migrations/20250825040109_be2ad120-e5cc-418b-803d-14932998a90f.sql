...
as $$
  select coalesce(
           (select has_premium from public.profiles where user_id = uid limit 1),
           (select has_premium from public.profiles where id      = uid limit 1),
           false
         );
$$;
...
-- Secure read policy: owners OR explicitly public OR premium users

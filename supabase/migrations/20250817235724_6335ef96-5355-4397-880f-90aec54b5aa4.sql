...
-- Data hygiene helpers (correct syntax)
do $$
begin
  if not exists (select 1 from information_schema.constraint_column_usage where constraint_name = 'chk_slug_not_blank') then
    alter table public.habit_template add constraint chk_slug_not_blank check (length(btrim(coalesce(slug,''))) > 0);
  end if;
end $$;

do $$
begin
  if not exists (select 1 from information_schema.constraint_column_usage where constraint_name = 'chk_name_not_blank') then
    alter table public.habit_template add constraint chk_name_not_blank check (length(btrim(coalesce(name,''))) > 0);
  end if;
end $$;

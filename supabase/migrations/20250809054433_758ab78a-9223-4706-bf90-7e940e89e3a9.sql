-- TEMP hydration-only debug logs in add_user_xp
...
BEGIN
  ...
  -- TEMP DEBUG (hydration only)
  IF base_reason = 'Hydrated' THEN
    RAISE LOG 'hydration_debug: before_check %',
      jsonb_build_object('base_reason', base_reason, 'ts', now());
  END IF;
  ...
  IF EXISTS (
    ...
    -- TEMP DEBUG (hydration only)
    IF base_reason = 'Hydrated' THEN
      RAISE LOG 'hydration_debug: duplicate_suppressed %',
        jsonb_build_object('base_reason', base_reason, 'ts', now());
    END IF;
  ...
  -- TEMP DEBUG (hydration only)
  IF base_reason = 'Hydrated' THEN
    RAISE LOG 'hydration_debug: final_award %',
      jsonb_build_object('final_award', final_award, 'ts', now());
  END IF;

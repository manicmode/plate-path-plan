...
AS $$
  ...
  meta AS (
    -- support both template and (future) custom habits if table exists
    SELECT
      m.slug,
    ...
    LEFT JOIN pg_catalog.pg_class pc
      ON pc.relname = 'user_custom_habit' -- existence guard
    LEFT JOIN public.user_custom_habit uch
      ON pc.oid IS NOT NULL
     AND uch.slug = m.slug
...
AS $$
BEGIN
  -- disable/remove reminders
  DELETE FROM public.habit_reminder
...
BEGIN
  -- purge reminders first
  DELETE FROM public.habit_reminder r

-- Security Cleanup Migration (Drop and Recreate Functions)

-- 1. Create severity_level enum first
DO $$ BEGIN
...
-- 4. Drop existing security functions
DROP FUNCTION IF EXISTS public.log_security_event(text, jsonb, uuid, text);
DROP FUNCTION IF EXISTS public.validate_security_event(text, uuid);
DROP FUNCTION IF EXISTS public.log_security_violation(text, jsonb, uuid, text);

-- 5. Create security functions with proper search_path
...
AS $$
DECLARE
  severity_enum severity_level;
BEGIN
  -- Convert text to enum
  severity_enum := severity_param::severity_level;
...
CREATE FUNCTION public.validate_security_event(
  event_type_param text,
  user_id_param uuid DEFAULT NULL
)
RETURNS boolean
...
CREATE FUNCTION public.log_security_violation(
...
-- 6. RLS Policies for security_events
...
DROP POLICY IF EXISTS "Admins can view all security violations" ON public.security_violations;
...
DROP POLICY IF EXISTS "System can insert security violations" ON public.security_violations;
CREATE POLICY "System can insert security violations"
ON public.security_violations
FOR INSERT
WITH CHECK (true);

-- 8. Drop and recreate SECURITY DEFINER views as RLS-compatible

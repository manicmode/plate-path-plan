...
-- 2) HABIT LOGS (each completion/check-in)
...
create index if not exists ul_user_date_idx on public.habit_log(user_id, logged_at desc);
create index if not exists ul_slug_idx on public.habit_log(slug);

alter table public.habit_log enable row level security;
drop policy if exists ul_owner_all on public.habit_log;
create policy ul_owner_all on public.habit_log
  using (user_id = auth.uid())
  with check (user_id = auth.uid());
...
begin
  ...
  insert into public.habit_log(user_id, slug, amount, duration_min, completed, meta)
...
-- 5) Progress views for weekly/monthly reporting

...
-- Add foreign key constraints (safe way)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'billboard_events_challenge_fk'
  ) THEN
    ALTER TABLE public.billboard_events
      ADD CONSTRAINT billboard_events_challenge_fk
      FOREIGN KEY (challenge_id) REFERENCES public.private_challenges(id) ON DELETE CASCADE;
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'billboard_comments_event_fk'
  ) THEN
    ALTER TABLE public.billboard_comments
      ADD CONSTRAINT billboard_comments_event_fk
      FOREIGN KEY (event_id) REFERENCES public.billboard_events(id) ON DELETE CASCADE;
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'billboard_reactions_event_fk'
  ) THEN
    ALTER TABLE public.billboard_reactions
      ADD CONSTRAINT billboard_reactions_event_fk
      FOREIGN KEY (event_id) REFERENCES public.billboard_events(id) ON DELETE CASCADE;
  END IF;
END $$;
...
DROP POLICY IF EXISTS "bc_select_members" ON public.billboard_comments;
DROP POLICY IF EXISTS "bc_insert_member_self" ON public.billboard_comments;
DROP POLICY IF EXISTS "bc_delete_own" ON public.billboard_comments;
DROP POLICY IF EXISTS "br_select_members" ON public.billboard_reactions;
DROP POLICY IF EXISTS "br_insert_member_self" ON public.billboard_reactions;
DROP POLICY IF EXISTS "br_delete_own" ON public.billboard_reactions;

...
/* activity days across nutrition sources (UTC day boundary via CURRENT_DATE; keep consistent with rest of DB) */
activity_days AS (
  SELECT DISTINCT user_id, created_at::date AS d FROM public.meal_scores      WHERE created_at::date <= CURRENT_DATE
  UNION
  SELECT DISTINCT user_id, created_at::date AS d FROM public.nutrition_logs   WHERE created_at::date <= CURRENT_DATE
  UNION
  SELECT DISTINCT user_id, created_at::date AS d FROM public.hydration_logs   WHERE created_at::date <= CURRENT_DATE
  UNION
  SELECT DISTINCT user_id, created_at::date AS d FROM public.supplement_logs  WHERE created_at::date <= CURRENT_DATE
),

/* standard streak grouping: date - row_number() OVER (ASC) */
streak_groups AS (
  SELECT
    user_id,
    d,
...
/* today's raw sub-scores with caps */
today_scores AS (
  SELECT 
    p.user_id,

    /* Meal scores: (avg(score)/100)*25, cap 25 */
    LEAST(
    ...
    /* Nutrition logs: (avg(quality_score)/100)*15, cap 15 */
    LEAST(
    ...
    /* Hydration: least(sum(volume)/2000,1)*5, cap 5  (column is hydration_logs.volume) */
    LEAST(
    ...
    /* Supplements: least(count(*),5)*1, cap 5 */
    LEAST(
...
scored AS (
  SELECT
    ...
    p.display_name,
    p.avatar_url,
    /* streak multiplier min(1 + 0.05 * streak, 1.5), category cap 50 */
    LEAST(

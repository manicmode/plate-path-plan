...
WHERE table_schema='public' AND table_name='runtime_flags'
ORDER BY grantee, privilege_type;

-- Active policies for flags + chat
SELECT tablename, polname, cmd, roles, qual, with_check
FROM pg_policies
WHERE schemaname='public'
  AND tablename IN ('runtime_flags','arena_chat_messages')
ORDER BY tablename, polname;

-- Realtime publication membership (flags + chat should both be listed)
SELECT *

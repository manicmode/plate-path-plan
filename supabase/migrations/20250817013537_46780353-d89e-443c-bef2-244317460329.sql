...
WITH enrolled AS (
  SELECT e.user_id, e.season_at
  FROM public.arena_enrollments e
  WHERE date_trunc('month', e.season_at) = date_trunc('month', now())
),
scores AS (
...
CREATE INDEX IF NOT EXISTS idx_arena_enrollments_month 
ON public.arena_enrollments (date_trunc('month', season_at));

...
SELECT
  ...
  0::numeric AS points,
  0::integer AS streak,
  ROW_NUMBER() OVER (ORDER BY COALESCE(p.display_name, p.first_name || ' ' || p.last_name, 'User') ASC, b.user_id ASC) AS rank,  -- 👈 stable rank
  COALESCE(p.display_name, p.first_name || ' ' || p.last_name, 'User') AS display_name,
  p.avatar_url
FROM base b

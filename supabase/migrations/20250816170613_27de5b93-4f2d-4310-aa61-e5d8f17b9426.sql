...
-- 1) Compute R20 candidate challenge IDs from backup (support title + type/category flags)
WITH candidates AS (
  SELECT b.id
  FROM _backup_private_challenges_20250816 b
  WHERE
    -- title heuristics (using actual column name)
    COALESCE(b.title, '') ILIKE ANY (ARRAY['%rank%', '%arena%', '%20%'])
 OR COALESCE(b.challenge_type, '') ILIKE '%rank%'
 OR COALESCE(b.category, '') ILIKE '%rank%'
)

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Logging sleep session for user: ${userId}`)

    // Get or create sleep streak record
    ...
    if (fetchError) {
      console.error('Error fetching sleep streak:', fetchError)
      throw fetchError
    }

    const today = new Date().toISOString().split('T')[0] // YYYY-MM-DD format
    console.log(`Processing sleep session for date: ${today}`)

    let updatedStreak

    if (!existingStreak) {
      // Create new streak record
      console.log('Creating new sleep streak record')
      ...
      if (error) {
        console.error('Error creating sleep streak:', error)
        throw error
      }

      updatedStreak = data
      console.log('Created new sleep streak:', updatedStreak)
    } else {
      // Update existing streak record
      console.log('Updating existing sleep streak:', existingStreak)
      ...
      if (lastCompletedDate === today) {
        console.log('Sleep session already completed today, no update needed')
      ...
      if (shouldUpdateToday) {
        ...
        if (error) {
          console.error('Error updating sleep streak:', error)
          throw error
        }

        updatedStreak = data
        console.log('Updated sleep streak:', updatedStreak)
      } else {
    ...
    return new Response(
      JSON.stringify({
        ...
        message: existingStreak?.last_completed_date === today 
          ? 'Sleep session already completed today' 
          : 'Sleep preparation logged successfully'
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )

  } catch (error) {
    console.error('Error in log-sleep-session:', error)
    return new Response(

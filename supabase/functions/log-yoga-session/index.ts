...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Logging yoga session for user: ${userId}`)

    // Get or create yoga streak record
    ...
    if (fetchError) {
      console.error('Error fetching yoga streak:', fetchError)
      throw fetchError
    }

    const today = new Date().toISOString().split('T')[0] // YYYY-MM-DD format
    console.log(`Processing yoga session for date: ${today}`)

    let updatedStreak

    if (!existingStreak) {
      // Create new streak record
      console.log('Creating new yoga streak record')
      ...
      if (error) {
        console.error('Error creating yoga streak:', error)
        throw error
      }

      updatedStreak = data
      console.log('Created new yoga streak:', updatedStreak)
    } else {
      // Update existing streak record
      console.log('Updating existing yoga streak:', existingStreak)
      ...
      if (shouldUpdateToday) {
        ...
        if (error) {
          console.error('Error updating yoga streak:', error)
          throw error
        }

        updatedStreak = data
        console.log('Updated yoga streak:', updatedStreak)
      } else {
    ...
    return new Response(
      JSON.stringify({
        ...
        message: existingStreak?.last_completed_date === today 
          ...
          : 'Yoga session logged successfully'
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )

  } catch (error) {
    console.error('Error in log-yoga-session:', error)
    return new Response(

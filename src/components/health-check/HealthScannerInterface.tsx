...
export const HealthScannerInterface: React.FC<HealthScannerInterfaceProps> = ({
  ...
  const startCameraFlow = useCallback(async () => {
    ...
    try {
      console.log("[CAMERA] Starting camera with getUserMedia...");
      ...
      <div className="relative flex flex-col min-h-dvh bg-black">
        ...
        <footer className="sticky bottom-0 z-40 bg-black/70 backdrop-blur-md px-4 pt-3 pb-safe">
          <ScannerActions
            ...
            torchEnabled={!!(trackRef.current?.getSettings() as any)?.torch}
            torchSupported={!!(trackRef.current?.getCapabilities() as any)?.torch}
          />
        </footer>
      </div>
    );

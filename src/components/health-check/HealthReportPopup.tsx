        ...
        <div className="text-center space-y-4">
          ...
          <Card className="bg-card border-border backdrop-blur-sm">
            <CardContent className="p-6">
              <h2 className="text-2xl font-bold text-foreground mb-4">{result.itemName}</h2>
              
              {/* Health Score */}
              <div className="flex items-center justify-center space-x-4 mb-4">
                <div className="text-center">
                  <div className="text-4xl font-bold text-foreground mb-2">{result.healthScore}</div>
            ...
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(result.nutritionData).map(([key, value]) => {
                  ...
                  return (
                    <div key={key} className="text-center p-3 bg-muted/50 rounded-lg border border-border">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-foreground">Organic: {result.healthProfile.isOrganic ? 'Yes' : 'No'}</span>
                </div>
                
                <div className="flex items-center space-x-2">
                  <XCircle className={`w-5 h-5 ${result.healthProfile.isGMO ? 'text-red-400' : 'text-green-400'}`} />
                  <span className="text-foreground">GMO: {result.healthProfile.isGMO ? 'Yes' : 'No'}</span>
                </div>
              </div>
              
              {result.healthProfile.allergens && result.healthProfile.allergens.length > 0 && (
                <div>
                  <div className="text-sm font-medium text-foreground mb-2">Allergens:</div>
              ...
              {result.healthProfile.additives && result.healthProfile.additives.length > 0 && (
                <div>
                  <div className="text-sm font-medium text-foreground mb-2">Additives:</div>
                  <div className="space-y-1">
              ...
              <div className="space-y-3">
                {result.ingredientFlags.map((flag, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg border border-border">
                    <div>
                      <div className="font-medium text-foreground">{flag.ingredient}</div>
                      <div className="text-sm text-muted-foreground">{flag.flag}</div>
                    </div>

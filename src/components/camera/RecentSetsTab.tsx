  ...
  if (loadingMealSets) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">Loading meal sets...</p>
      </div>
    );
  }

  if (recentMealSets.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="w-16 h-16 bg-muted rounded-full flex items-center justify-center mx-auto mb-4">
          <Package className="h-8 w-8 text-muted-foreground" />
        </div>
        <h3 className="text-lg font-semibold mb-2">No Recent Meal Sets</h3>
        <p className="text-muted-foreground mb-4">
          Create meal sets to see them here for quick logging
        </p>
      </div>
    );
  }

  ...
  const startCamera = async () => {
    ...
    try {
      ...
      if (isIOSWebKit) {
        ...
        console.log('[LOG][iOS] Using relaxed constraints for torch compatibility');
      } else {
      ...
      console.log('[LOG] Initial stream settings:', {
      ...
      if (isIOSWebKit && !hasTorch) {
        console.log('[LOG][iOS] No torch on initial camera, attempting retry with alternate device');
        try {
          // Stop current stream
          ...
          console.log('[LOG][iOS] Available video devices:', videoDevices.map(d => ({ 
            label: d.label, 
          ...
          for (const device of videoDevices) {
            if (device.label.toLowerCase().includes('back') || 
              ...
              try {
                console.log(`[LOG][iOS] Trying device: ${device.label}`);
                ...
                console.log(`[LOG][iOS] Device ${device.label} torch support:`, retryHasTorch);
                
                if (retryHasTorch) {
                  console.log(`[LOG][iOS] Found torch-capable camera: ${device.label}`);
                ...
                console.warn(`[LOG][iOS] Failed to access device ${device.label}:`, deviceError);
              }
            }
          }
        } catch (enumerateError) {
          console.warn('[LOG][iOS] Device enumeration failed:', enumerateError);
        }
      }
      
      console.log('[LOG] Final stream settings:', {
        width: settings.width,
        height: settings.height,
        facingMode: settings.facingMode,
        deviceId: settings.deviceId

...
export function NudgeCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        // Container - Light mode (≥80% opacity for visibility)
    ...
    >
      <div className="relative z-10">
        ...
        <div className="flex items-start gap-3 mb-3">
          {icon && (
            <div 
              className="h-9 w-9 shrink-0 rounded-xl bg-gradient-to-tr from-indigo-500 to-fuchsia-500 text-white shadow-[0_8px_18px_-6px_rgba(99,102,241,0.55)] flex items-center justify-center"
          ...
          {onDismiss && (
            <button
              onClick={onDismiss}
              className="h-8 w-8 shrink-0 rounded-lg flex items-center justify-center bg-slate-900/5 dark:bg-white/5 text-slate-600 dark:text-slate-300 hover:bg-slate-900/10 dark:hover:bg-white/10 transition-colors"
        ...
        {cta && (
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              onClick={cta.onClick}
              className="flex items-center gap-2 rounded-xl px-4 py-2 min-h-[44px] bg-gradient-to-tr from-indigo-500 to-violet-500 text-white font-medium shadow-[0_10px_24px_-10px_rgba(99,102,241,0.65)] hover:opacity-95 active:scale-[0.99] transition-all duration-150"

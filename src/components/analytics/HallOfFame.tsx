...
export const HallOfFame: React.FC<HallOfFameProps> = ({ 
  ...
  const [isPostingTribute, setIsPostingTribute] = useState(false);
  ...
  console.log('Tributes from hook:', tributes);
  ...
  const handleSubmitTribute = async () => {
    ...
    if (!user) {
      toast({
        ...
        description: "Please log in to post a tribute.",
        variant: "destructive"
      });
      return;
    }
    ...
    if (!championUserId) {
      toast({
        title: "No Champion Available",
        description: "No Hall of Fame champion found for this year yet.",
        variant: "destructive"
      });
      return;
    }
    ...
    console.log('Attempting to post tribute:', {
      message: newTribute.trim(),
      championUserId,
      userId: user.id
    });

    setIsPostingTribute(true);
    
    try {
      const success = await postTribute(newTribute.trim());
      
      if (success) {
        console.log('Tribute posted successfully');
        setNewTribute('');
        toast({
          title: "Tribute Posted!",
          description: "Your congratulatory message has been posted successfully.",
        });
      } else {
        console.error('Failed to post tribute - postTribute returned false');
        toast({
          title: "Failed to Post",
          description: "There was an issue posting your tribute. Please try again.",
          variant: "destructive"
        });
      }
    } catch (error) {
      ...
      <Card className="overflow-hidden border-2 border-amber-200 shadow-xl bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 dark:from-amber-950/20 dark:via-yellow-950/20 dark:to-orange-950/20">
        <CardHeader className="bg-gradient-to-r from-amber-100 to-yellow-100 dark:from-amber-900/30 dark:to-yellow-900/30 text-center relative overflow-hidden">
          ...
          <p className="text-sm text-muted-foreground mt-2 relative z-10">
            {hasChampions ? "Legends who made their mark in nutrition history" : "Coming soon - champions will be announced!"}
          </p>
        </CardHeader>
        ...
        <CardContent className="p-6">
          {hasChampions ? (
            ...
            <div className="text-center py-16">
              <Trophy className="h-20 w-20 mx-auto mb-4 text-muted-foreground/30" />
              <h3 className="text-xl font-semibold mb-2">Hall of Fame Coming Soon</h3>
              <p className="text-muted-foreground">
                Champions will be announced at the end of each month based on their performance and achievements.
              </p>
            </div>
          )}
        </CardContent>

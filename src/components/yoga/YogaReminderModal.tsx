...
interface YogaReminderModalProps {
  isOpen: boolean;
  onClose: () => void;
  reminder?: {
    id: string;
...
export const YogaReminderModal = ({ isOpen, onClose, reminder }: YogaReminderModalProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (reminder) {
        ...
        toast({
          title: "Success",
          description: "Yoga reminder updated successfully"
        });
      } else {
        // Create new reminder
        const { error } = await supabase
        ...
        toast({
          title: "Success",
          description: "Yoga reminder created successfully"
        });
      }

      onClose();
    } catch (error) {
      console.error('Error saving yoga reminder:', error);
      toast({
        title: "Error",
        description: "Failed to save yoga reminder. Please try again.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {reminder ? 'Edit Yoga Reminder' : 'Set Yoga Reminder'}
          </DialogTitle>
        </DialogHeader>

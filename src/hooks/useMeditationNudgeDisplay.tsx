...
interface NudgeDisplayHook {
  recentNudges: MeditationNudge[]
  activeNudge: MeditationNudge | null
  loading: boolean
...
export const useMeditationNudgeDisplay = (): NudgeDisplayHook => {
  const [recentNudges, setRecentNudges] = useState<MeditationNudge[]>([])
  ...
  const fetchNudges = useCallback(async () => {
    try {
      ...
      if (error) {
        console.error('Error fetching meditation nudges:', error)
        return
      }
  ...
  const acceptNudge = async (nudgeId: string) => {
    ...
    if (success) {
      toast({
        ...
        description: "Great choice for your wellbeing"
      })
    }
  }

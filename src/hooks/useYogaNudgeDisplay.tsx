...
interface NudgeDisplayHook {
  recentNudges: YogaNudge[]
  activeNudge: YogaNudge | null
  loading: boolean
...
export const useYogaNudgeDisplay = (): NudgeDisplayHook => {
  const [recentNudges, setRecentNudges] = useState<YogaNudge[]>([])
  ...
  const fetchNudges = useCallback(async () => {
    try {
      ...
      if (error) {
        console.error('Error fetching yoga nudges:', error)
        return
      }
  ...
  const acceptNudge = async (nudgeId: string) => {
    ...
    if (success) {
      toast({
        title: "Let's flow! 🧘‍♀️",
        description: "Perfect time for some yoga"
      })
    }
  }
  ...
  useEffect(() => {
    ...
    // Set up real-time subscription for yoga nudges
    const channel = supabase
      .channel('yoga-nudges-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        () => {
          console.log('Yoga nudge updated, refreshing...')
          fetchNudges()
        }
      )
      .subscribe()

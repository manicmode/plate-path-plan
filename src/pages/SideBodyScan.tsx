...
export default function SideBodyScan() {
    ...
    const detectPoseRealTime = async (currentTime: number) => {
      ...
      try {
        ...
        if (poses.length > 0) {
          ...
          // Analyze alignment for side view pose
          const alignment = analyzeSidePoseAlignment(pose);
  ...
  const captureImage = async () => {
    ...
    toast({
      ...
      description: "Side body scan complete. Ready to continue to back scan.",
    });
  };
  ...
  const saveBodyScanToSupabase = async (imageDataUrl: string) => {
    try {
      ...
      // Create filename with timestamp as requested: ${user_id}/side-${Date.now()}.jpg
      const timestamp = Date.now();
      const fileName = `${user.id}/side-${timestamp}.jpg`;
      ...
      // Calculate pose metadata with specific structure for side view
      const poseMetadata = {
        shouldersLevel: !alignmentFeedback?.misalignedLimbs.includes('shoulders'),
        armsRaised: alignmentFeedback?.misalignedLimbs.includes('arms_position') ? false : true,
      ...
      // Save entry to body_scans table with type: 'side'
      ...
      await triggerScanCompletedNotification('side');
      await showInstantFeedback('side');
      ...
      toast({
        title: "Side Scan Saved ✅",
        description: "Your side body scan has been securely saved.",
      });
  ...
  // Side pose analysis functions
  const analyzeSidePoseAlignment = useCallback((pose: DetectedPose): AlignmentFeedback => {
    const alignmentThreshold = 0.15; // 15% tolerance
    const misalignedLimbs: string[] = [];
    let feedback = "";
    
    // Find key landmarks for side view
    ...
    // Check if person is in profile (side view) - one shoulder should be significantly more visible
    ...
    // Check hip visibility - one hip should be more visible than the other
    const leftHipScore = leftHip?.score || 0;
    const rightHipScore = rightHip?.score || 0;
    const hipScoreDiff = Math.abs(leftHipScore - rightHipScore);
    
    if (hipScoreDiff < 0.2) {
      misalignedLimbs.push('hip_profile');
      feedback = "Stand more to your side";
    }
    
    // Check arm position - arms should be along the side (not raised)
    const visibleShoulder = leftShoulderScore > rightShoulderScore ? leftShoulder : rightShoulder;
    const visibleWrist = leftShoulderScore > rightShoulderScore ? leftWrist : rightWrist;
    
    if (visibleShoulder && visibleWrist && visibleShoulder.score > 0.5 && visibleWrist.score > 0.3) {
      const armHeight = Math.abs(visibleWrist.y - visibleShoulder.y);
      const armWidth = Math.abs(visibleWrist.x - visibleShoulder.x);
      
      // In side view, wrist should be below or near shoulder level
      if (visibleWrist.y < visibleShoulder.y - 50) { // wrist is above shoulder
        misalignedLimbs.push('arms_position');
        feedback = "Lower your arms to your sides";
      }
    }
    
    // Check feet alignment - ankles should be close together (stacked in profile)
    if (leftAnkle && rightAnkle && leftAnkle.score > 0.4 && rightAnkle.score > 0.4) {
      const ankleDistance = Math.abs(leftAnkle.x - rightAnkle.x);
      const videoWidth = videoRef.current?.videoWidth || 640;
      const relativeDistance = ankleDistance / videoWidth;
      
      if (relativeDistance > 0.1) { // 10% of screen width
        misalignedLimbs.push('feet_stacked');
        feedback = "Stand with feet together (profile view)";
      }
    }
    
    // Check body centering
    const visibleHip = leftHipScore > rightHipScore ? leftHip : rightHip;
    if (visibleHip && visibleHip.score > 0.5) {
      const screenCenter = (videoRef.current?.videoWidth || 640) / 2;
      const centerOffset = Math.abs(visibleHip.x - screenCenter) / screenCenter;
    ...
    const totalCheckpoints = 5; // profile, hip_profile, arms_position, feet_stacked, centering
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      toast({
        ...
        description: "Side body image ready. You can now continue to the back scan.",
      });
    };
  ...
  const handleContinue = () => {
    if (hasImageReady && savedScanUrl) {
      ...
      sessionStorage.setItem('sideBodyScanUrl', savedScanUrl);
      navigate('/body-scan-back');
    }
  };
      ...
      <div className="absolute top-4 md:top-6 left-4 right-4 z-20">
        <div className="bg-black/60 backdrop-blur-sm rounded-2xl p-4 border border-white/30">
          ...
          <p className="text-white/90 text-sm text-center">
            Stand sideways with arms at your sides. Match your profile to the outline.
          </p>
        </div>
      </div>

      
      {/* Body Silhouette Overlay - Mirrored for side view */}
      <div className="absolute inset-0 flex items-center justify-center mt-[-4vh] z-15">
        <div className={`relative transition-all duration-500 ${
          ...
          <img 
            ...
            style={{ 
              transform: 'scaleX(-1)', // Mirror the image for side view
      ...
      {showSuccessScreen && savedScanUrl && (
        <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-30 p-6">
          <div className="bg-white/10 backdrop-blur-md rounded-3xl p-6 text-center max-w-sm">
            <div className="text-4xl mb-4">✅</div>
            <h3 className="text-white text-xl font-bold mb-4">Side Scan Saved!</h3>
        ...
        <div className="absolute top-1/2 left-4 right-4 z-25 transform -translate-y-1/2">
          <div className="bg-green-500/90 backdrop-blur-sm rounded-2xl p-4 border border-green-400">
            <div className="text-center">
              ...
              <p className="text-white font-bold text-lg">Perfect Side Pose!</p>

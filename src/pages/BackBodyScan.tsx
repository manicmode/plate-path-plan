...
export default function BackBodyScan() {
    ...
    const detectPoseRealTime = async (currentTime: number) => {
      ...
      try {
        ...
        if (poses.length > 0) {
          ...
          // Analyze alignment for back view pose
          const alignment = analyzeBackPoseAlignment(pose);
  ...
  const captureImage = async () => {
    ...
    toast({
      ...
      description: "Back body scan complete. Body scan session finished!",
    });
  };
  ...
  const saveBodyScanToSupabase = async (imageDataUrl: string) => {
    try {
      ...
      // Create filename with timestamp as requested: ${user_id}/back-${Date.now()}.jpg
      const timestamp = Date.now();
      const fileName = `${user.id}/back-${timestamp}.jpg`;
      ...
      // Calculate pose metadata with specific structure for back view
      const poseMetadata = {
      ...
      // Save entry to body_scans table with type: 'back'
      ...
      toast({
        ...
        description: "Your back body scan has been securely saved.",
      });
  ...
  // Back pose analysis functions
  const analyzeBackPoseAlignment = useCallback((pose: DetectedPose): AlignmentFeedback => {
    const alignmentThreshold = 0.15; // 15% tolerance
    const misalignedLimbs: string[] = [];
    let feedback = "";
    
    // Find key landmarks for back view
    ...
    // Check if person is facing away (back view) - nose should be less visible or head turned away
    if (nose && nose.score > 0.6) {
      misalignedLimbs.push('face');
      feedback = "Please turn around to face away from camera";
    }
    
    // Check if both shoulders are visible (essential for back view)
    if (!leftShoulder || !rightShoulder || leftShoulder.score < 0.4 || rightShoulder.score < 0.4) {
      misalignedLimbs.push('shoulders');
      feedback = "Position yourself so both shoulders are visible";
    }
    
    // Analyze shoulder alignment (should be horizontal like front view)
    if (leftShoulder && rightShoulder && leftShoulder.score > 0.4 && rightShoulder.score > 0.4) {
    ...
    // Check arm position - arms should be relaxed or slightly raised (not blocking back)
    if (leftWrist && rightWrist && leftShoulder && rightShoulder) {
      const leftArmVisible = leftWrist.score > 0.3;
      const rightArmVisible = rightWrist.score > 0.3;
      
      // At least one arm should be visible, preferably both
      if (!leftArmVisible && !rightArmVisible) {
        misalignedLimbs.push('arms_position');
        feedback = "Move arms away from your back";
      } else {
        // Check if arms are too high (blocking view of back)
        if (leftArmVisible && leftShoulder.score > 0.4) {
          const leftArmHeight = leftWrist.y - leftShoulder.y;
          if (leftArmHeight < -100) { // arm too high above shoulder
            misalignedLimbs.push('arms_position');
            feedback = "Lower your arms slightly";
          }
        }
        
        if (rightArmVisible && rightShoulder.score > 0.4) {
          const rightArmHeight = rightWrist.y - rightShoulder.y;
          if (rightArmHeight < -100) { // arm too high above shoulder
            misalignedLimbs.push('arms_position');
            feedback = "Lower your arms slightly";
          }
        }
    ...
    const totalCheckpoints = 5; // face (turned away), shoulders, arms_position, centering, shoulders_level
    const alignmentScore = Math.max(0, (totalCheckpoints - misalignedLimbs.length) / totalCheckpoints);
    
    if (misalignedLimbs.length === 0) {
      feedback = "Perfect back pose! Hold steady...";
    }
  ...
  const drawPoseOverlay = useCallback((pose: DetectedPose, alignment: AlignmentFeedback) => {
    ...
    pose.keypoints.forEach((keypoint) => {
      if (keypoint.score > 0.4) { // Slightly lower threshold for back view
    ...
    // Draw skeleton connections for back view
    const connections = [
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      toast({
        ...
        description: "Back body image ready. You can now finish the body scan.",
      });
    };
  ...
  const handleFinish = () => {
    if (hasImageReady && savedScanUrl) {
      // Store the saved scan URL
      sessionStorage.setItem('backBodyScanUrl', savedScanUrl);
    ...
    <div className="fixed inset-0 w-full h-full bg-black overflow-hidden portrait:block landscape:hidden">
      ...
      <div className="absolute top-4 md:top-6 left-4 right-4 z-20">
        <div className="bg-black/60 backdrop-blur-sm rounded-2xl p-4 border border-white/30">
          ...
          <p className="text-white/90 text-sm text-center">
            Turn around and face away from camera. Keep arms relaxed at your sides.
          </p>
        </div>
      </div>

      
      {/* Body Silhouette Overlay - Back-facing view */}
      ...
      {showSuccessScreen && savedScanUrl && (
        <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-30 p-6">
          <div className="bg-white/10 backdrop-blur-md rounded-3xl p-6 text-center max-w-sm">
            <div className="text-4xl mb-4">✅</div>
            <h3 className="text-white text-xl font-bold mb-4">Back Scan Saved!</h3>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 via-black/70 to-transparent z-20">
        <div className="flex flex-col space-y-4">
          ...
          {/* Main Action Button */}
          <Button
            onClick={hasImageReady ? handleFinish : captureImage}
            disabled={
              isCapturing || 
              isSaving ||
        ...
        <div className="absolute top-1/2 left-4 right-4 z-25 transform -translate-y-1/2">
          <div className="bg-green-500/90 backdrop-blur-sm rounded-2xl p-4 border border-green-400">
            <div className="text-center">
              ...
              <p className="text-white font-bold text-lg">Perfect Back Pose!</p>
